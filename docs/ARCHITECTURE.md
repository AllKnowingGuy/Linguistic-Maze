...
или изменить FPS. Отрисовка разделена на два слоя: content_layer (игровой контент) и hud_layer (FPS). Состояние может 
указывать, когда нужно обновлять экран, что экономит ресурсы.
...
вызывает associate_current_state(), чтобы подставить нужный объект состояния. Команды от состояний 
(например, CHECK_PROGRESS) заставляют StoryScript обновить прогресс.

Загрузка ресурсов

AssetsCreation загружает все спрайты, звуки и шрифты. Каждая функция загрузки возвращает pygame.Surface или 
pygame.mixer.Sound. Состояния хранят ссылки на нужные ресурсы и используют их в draw. Transformer позволяет получать 
отражённые или повёрнутые версии спрайтов без повторной загрузки.

Лабиринт

MazeState генерирует лабиринт через Maze.generate_maze. При движении игрока проверяет коллизии со стенами (через массив 
pattern) и врагами. При столкновении с врагом вызывается StoryScript.handle_monster_encounter, который переключает 
состояние на DIALOGUE. При достижении выхода – handle_room_exit.

Диалог

DialogueState загружает JSON через Dialogue. В зависимости от реплики отображает текст, портреты, кнопки выбора или 
поле ввода. Пользователь может пропустить анимацию нажатием Esc. После завершения реплики или выбора варианта 
DialogueState отправляет команду CHECK_PROGRESS, что вызывает StoryScript.handle_dialogue_finish. Если диалог ведёт к 
испытанию (starts_challenge), переключается на CHALLENGE.

Задание (Challenge)

ChallengeState загружает JSON через Challenge. Показывает окна по очереди, сохраняет ответы. При отправке проверяет 
через Challenge правильность каждого ответа (используя чекеры). Обновляет респекты и артефакты в StoryScript.rooms_data. После завершения задания возвращается в лабиринт.

Настройки управления

Config читает config.json. В MenuState кнопки управления вызывают диалог ожидания нажатия. После ввода новая клавиша сохраняется в Config и записывается в файл. При запуске лабиринта или диалога Main передаёт привязки в соответствующие состояния, которые используют их в обработке ввода.

Обработка ввода

Все состояния наследуют методы handle_input, handle_hold_input, handle_button_release, handle_mouse_motion, handle_mouse_click, handle_mouse_release от BaseState. Main вызывает эти методы, передавая события. Состояния возвращают списки команд. Это позволяет гибко реагировать на клавиатуру и мышь, сохраняя полиморфизм.

Оптимизация отрисовки

Состояния используют флаг needs_screen_update, чтобы перерисовывать только при изменении содержимого (движение игрока, появление нового текста и т.д.). ChallengeState и DialogueState кэшируют большие статичные элементы (фон, диалоговое окно) в отдельных поверхностях и блютят их только при необходимости.

Цепочка команд

Команды – это кортежи (Command, параметр). Main обрабатывает их после каждого этапа обработки ввода и отрисовки. Это позволяет, например, из состояния диалога установить новый FPS, из лабиринта – проверить прогресс, а из любого состояния – остановить игру.