...
class ChallengeState(BaseState):
    ...
    def __init__(self, checker: Checker):
        ...
        self.back_button = Button(
            BACK_BUTTON_X, NAV_BUTTON_Y, CHAL_BUTTON_WIDTH, CHAL_BUTTON_HEIGHT, "back"
        )
        self.forth_button = Button(
            FORTH_BUTTON_X, NAV_BUTTON_Y, CHAL_BUTTON_WIDTH, CHAL_BUTTON_HEIGHT, "forth"
        )
        ...
        # Заставки
        self.start_cover, self.check_cover, self.end_cover = (
            assetscreation.add_transitions()
        )
        ...
        self.roll_sound = assetscreation.add_roll_sound()
    ...
    def add_sounds_for_volume_change(self):
        """
        Возврат звуков испытания для изменения громкости
        """
        ...
        return (
            (
                Command.ADD_SOUNDS,
                {
                    self.start_challenge_sound,
                    self.transition_sound,
                    self.roll_sound,
                    self.correct_stamp_sound,
                    self.incorrect_stamp_sound,
                },
            ),
        )
    ...
    def setup_challenge(self, json_path: Path):
        """
        Задание структурных данных испытания, сброс параметров прошлого испытания

        Args:
            json_path (Path): путь к структурному файлу испытания
        """

        self.challenge = Challenge(json_path)
        self.score = 0
        ...
        # Обновление клавиши прекращения анимации (чтобы подтянулись изменения после настройки)
        self.stop_anim_bind = Config().get_challenge_controls()
    ...
    def change_card(self, back: bool = False):
        ...
        Args:
            back (bool): нужно ли переключаться на предыдущее задание
        """
    ...
    def change_card_on_checking(self):
        ...
        self.current_window_ind += 1
        while not self.challenge.get_window_action_type(
            self.current_window_ind
        ) or self.challenge.get_window_action_type(self.current_window_ind) not in (
            "choosefrom",
            "savetyped",
        ):
            self.current_window_ind += 1
    ...
    def get_window_fields(self):
        ...
        # Извлекаем текст
        self.current_task_text = self.challenge.get_window_task_text(
            self.current_window_ind
        )
        self.text_line = 0
        ...
        if image_path:
            if not image_path in self.image_cache:
                self.image_cache[image_path] = assetscreation.add_window_image(
                    image_path
                )
        ...
        if not self.submitted:
            if (
                self.current_window_ind == 0
                and self.current_window_ind == len(self.challenge.windows) - 1
            ):
    ...
    def set_choosefrom_window(self):
        """Создание кнопок для вопроса с вариантами выбора"""

        if self.current_window_ind in self.choice_buttons_sets:
            return

        self.choice_buttons_sets[self.current_window_ind] = []
        current_choice_options = list(
            set(self.challenge.get_window_choose_options(self.current_window_ind))
        )
    ...
    def start_start_anim(self):
        """Начало анимации загрузки испытания"""
    ...
    def play_start_anim(self):
        if self.playing_start_anim:
            ...
            if current_ticks >= self.start_anim_time + TRANSITION_TIME:
                ...
                self.start_anim_perc = float(
                    (current_ticks - self.start_anim_time)
                    // (TRANSITION_TIME // 16)
                    * 6.25
                    + 6.25
                )
    ...
    def start_check_anim(self):
        """Начало анимации перехода к проверке"""
    ...
    def play_check_anim(self):
        if (
            self.playing_check_load_anim
            and pygame.time.get_ticks() >= self.check_load_anim_time + TRANSITION_TIME
        ):
            self.playing_check_load_anim = False
    ...
    def start_window_check_anim(self):
        """Начало анимации проверки одного окна"""
    ...
    def play_window_check_anim(self):
        if not self.playing_window_check_anim:
            return

        # Анимация и звук штампа правильности
        current_ticks = pygame.time.get_ticks()
        if (
            self.current_answer_correctness is None
            ...
            user_input = self.challenge.answers[self.current_window_ind]
            ...
            self.current_answer_correctness = self.checker.check(
                user_input, keys, checker
            )

            self.current_reward = (
                self.challenge.get_window_incorrect_respect_points(
                    self.current_window_ind
                ),
                self.challenge.get_window_correct_respect_points(
                    self.current_window_ind
                ),
        ...
        elif current_ticks >= self.check_window_anim_time + CHECK_TIME + COMMENT_TIME:

            self.current_tip = random.choice(
                (
                    self.challenge.get_window_incorrect_tips(self.current_window_ind),
    ...
    def start_end_anim(self):
        """Начало анимации завершения испытания"""
        self.transition_sound.play()
    ...
    def play_end_anim(self):
        if (
            not self.finished
            and pygame.time.get_ticks() >= self.end_anim_time + RESULTS_TIME
        ):
            self.finished = True
            self.need_screen_update = True
    ...
    def handle_input(self, event: pygame.event.Event):
        ...
        # Ограничения
            ...
            self.awaiting.name == Awaiting.INPUT.name
            ...
            self.input_texts[self.current_window_ind], updated = (
    ...
    def handle_mouse_motion(self, event):
        ...
        if self.playing_start_anim or self.playing_check_load_anim or self.verdicted:
            return
    ...
    def handle_mouse_click(self, event):
        ...
        if (
            ...
            or self.playing_start_anim
            or self.playing_check_load_anim
            or self.verdicted
        ):
            return
    ...
    def handle_mouse_release(self, event):
        ...
        if (
            ...
            or self.playing_check_load_anim
            or self.verdicted
            or not ButtonState.PRESSED in [btn.state for btn in self.nav_buttons]
        ):
            return
    ...
    def draw(self, screen):
        ...
        if self.need_cache:
            ...
            self.bg_card_task_img_cache.blit(
                self.question_card,
                (
                    get_centered_point(self.question_card.get_width(), False),
                    TITLE_Y - 85,
                ),
            )

            # Отрисовываем заголовок
            title_sprite = self.ps_font.render(self.current_title, True, (0, 0, 0))
            self.bg_card_task_img_cache.blit(
                title_sprite,
                (get_centered_point(title_sprite.get_width(), False), TITLE_Y),
            )
            ...
            if self.current_image:
                self.bg_card_task_img_cache.blit(
                    self.current_image,
                    (
                        get_centered_point(self.current_image.get_width(), False),
                        IMAGE_Y,
                    ),
                )
        ...
        btn_sprites_set = (
            self.back_button_sprites,
            self.forth_button_sprites,
            self.submit_button_sprites,
        )
        ...
        # Отрисовываем текст ошибки отправки
        if self.failed_to_submit:
            fail_text_sprite = self.ps_font.render(
                "Выполни все задания!", True, (108, 0, 0)
            )
            screen.blit(
                fail_text_sprite,
                (FORTH_BUTTON_X - fail_text_sprite.get_width() - 10, NAV_BUTTON_Y + 30),
            )
        ...
        if not self.playing_text or self.submitted:
            ...
            if self.awaiting.name == Awaiting.CHOOSE.name:
                for btn in self.choice_buttons_sets[self.current_window_ind]:
                    ...
                    # Текст кнопок
                    input_text_sprite = self.ps_font.render(btn.text, True, (0, 0, 0))
            ...
            # Отрисовываем вводимый текст (если можно вводить)
                ...
                input_text_sprite = self.ps_font.render(
        ...
        if self.current_stamp:
            screen.blit(
                self.current_stamp,
                (
                    get_centered_point(self.current_stamp.get_width(), False),
                    NAV_BUTTON_Y - 25,
                ),
            )
        ...
        if self.current_tip:
            screen.blit(
                self.tip_card,
                (
                    get_centered_point(self.tip_card.get_width(), False),
            ...
            tip_text = self.ps_font.render(self.current_tip, True, (0, 0, 0))
            screen.blit(
                tip_text,
                (
                    get_centered_point(self.tip_card.get_width(), False) + 40,
                    get_centered_point(self.tip_card.get_height(), True) + 40,
                ),
            )
            reward_announce_text = self.ps_font.render(reward_announce, True, (0, 0, 0))
            screen.blit(
                reward_announce_text,
                (
                    get_centered_point(self.tip_card.get_width(), False) + 40,
        ...
        if self.playing_start_anim:
            ...
            percent_text = self.ps_font.render(
                f"{int(self.start_anim_perc)} %", True, (255, 255, 255)
            )
            screen.blit(
                percent_text,
                (
                    SCREEN_WIDTH - 200 - percent_text.get_width(),
                    SCREEN_HEIGHT - 100,
                ),
            )
        elif self.playing_check_load_anim:
            ...
            score_text = self.ps_font.render(
                f"Респект, который ты заработал: {self.score}",
                True,
                (255, 255, 255),
            )
            screen.blit(
                score_text,
                (
                    get_centered_point(score_text.get_width(), False),
                    SCREEN_HEIGHT / 2 + 100,
                ),
            )
        ...
        return (
            (Command.CHECK_PROGRESS, None),
            (Command.UPDATE_DISPLAY, None),
        )
    ...
    def check_save_and_submit(self):
        """Проверка выполненности всех заданий, сохранение ответов и переход к проверке"""

        # Проверяем, что все поля ввода не пустые и в каждой группе кнопок есть нажатая
    ...
    def draw_text_by_letter(self, screen):
        ...
        for i in range(self.text_line):
            text_sprite = self.ps_font.render(
        ...
        text_sprite = self.ps_font.render(
            self.current_task_text[self.text_line][: int(self.text_cursor)],
            True,
            (0, 0, 0),
        )
