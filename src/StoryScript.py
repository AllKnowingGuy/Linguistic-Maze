...
class StoryScript:
    ...
    def initiate_progress(self):
        ...
        for i in range(3): # TODO: add all rooms
            with open(f'..\\assets\\data\\rooms\\level_{i}.json') as f:
    ...
    def update_game_progress(self, game: Main):
        ...
        # Когда игрок на испытании
            ...
            if game.challenge_state.finished:
                ...
                supposed_end_dialogue = f'..\\assets\\data\\dialogues\\level_{self.current_room}\\{self.on_monster.enemy_name}_end.json'
    ...
    def handle_intro_dialogue(self, game: Main):
        ...
        check_line = ("Перед тобой четыре ФиПЛовца, которые отважно решились пройти практику к концу обучения "
                      "на 3-м курсе.\nКто тебе нравится больше всего?")
        if game.dialogue_state.current_line == check_line.split("\n"):
            self.intro_progress["choosing_character"] = True
        if self.intro_progress["choosing_character"] and game.dialogue_state.current_line != check_line.split("\n"):
            chosen_name = game.dialogue_state.dialogue.saved_choices.get(check_line, "Аня")
            ...
            self.intro_progress["choosing_character"] = False
    ...
    def handle_door_dialogue(self, game: Main):
        ...
        if game.dialogue_state.current_line == ["Тогда введи код!"]:
            self.door_progress["entering_code"] = True
        if self.door_progress["entering_code"] and game.dialogue_state.current_line != ["Тогда введи код!"]:
            code_guess = game.dialogue_state.dialogue.saved_inputs.get("Тогда введи код!", "")
            if code_guess != self.current_door_code:
                game.dialogue_state.current_line_ind += (1 if self.attempts_left > 0 else 2)
            self.door_progress["entering_code"] = False
        ...
        if game.dialogue_state.current_line == [fate_text]:
            self.door_progress["choosing_fate"] = True
        if (self.door_progress["choosing_fate"]
                and game.dialogue_state.current_line != [fate_text]):
            fate = game.dialogue_state.dialogue.saved_choices.get(fate_text, "Пойду поищу")
            if fate == "Никак не могу найти...":
                game.dialogue_state.advance(
                    game.dialogue_state.current_line_ind + (0 if self.attempts_left > 0 else 1)
                )
            self.door_progress["choosing_fate"] = False
    ...
    def handle_dialogue_finish(self, game: Main):
        ...
        # Если это диалог двери выхода
            ...
            if self.current_room == 2:
                ...
                # print(f"ON LINE {line_ind} THERE ARE {game.dialogue_state.dialogue.get_line_room_switch(line_ind)} AND {game.dialogue_state.dialogue.get_line_room_restart(line_ind)}")
                ...
                elif game.dialogue_state.current_line_ind == len(game.dialogue_state.dialogue.lines) - 1:
                    game.running = False # TODO: this is temporary until I make a gameover
                else:
                    game.current_state_type = StateType.MAZE
                    game.associate_current_state()
    ...
    def collect_dialogue_bonuses(self, game: Main):
        ...
        line_ind = game.dialogue_state.current_line_ind
        if not game.dialogue_state.finished and line_ind > 0:
            line_ind -= 1
        ...
        if (self.current_room in self.rooms_data
                and not game.dialogue_state.dialogue.collected_respect_points.get(line_ind, False)):
    ...
    def make_door_interaction(self, room_data):
        ...
        if not stop_building and artifacts_req and got_artifact:
            door_dialogue["lines"].append(
                {
                    ...
                    f"лежит {artifacts_req[0]}" if len(artifacts_req) == 1 else f"лежат {", ".join(artifacts_req)}"
            ...
            door_dialogue["lines"].append(
                {
                    "text": f"Но если тебе {
                    f"попадётся {artifacts_req[0]}" if len(artifacts_req) == 1 else f"попадутся {", ".join(artifacts_req)}"
                    }, и ты это принесёшь сюда, то я сделаю тебе скидку на респект."
                }
            )
            ...
            door_dialogue["lines"].extend([
                ...
                },
                {
                    "text": f"Код неверный!\nТеперь тебе придётся пройти комнату заново, но она будет воссоздана. "
                            f"У тебя осталось {f"2 воссоздания!" if self.attempts_left >= 2 else f"1 воссоздание!"}",
                    "jump": -1,
                    "restartroom": True
                },
                {
        ...
        if scored_respect or scored_discounted_respect:
            ...
            door_dialogue["lines"].extend([
                ...
                {
                    "text": f"Прискорбно! Придётся воссоздать комнату, пройди её ещё раз.\nУ тебя осталось {
                    f"2 воссоздания!" if self.attempts_left >= 2 else f"1 воссоздание!"
                    }",
                    "jump": -1,
                    "restartroom": True
                },
