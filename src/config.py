...
def extract_decimal_code(bind: str):
    if len(bind) == 1:
        ...
        raise ValueError(f'String {bind} is not a single character or a 4-digit hexadecimal Unicode code.')
...
class Config:
    ...
    def set_controls(self, controls: dict[str, str]):
        if all([bind in controls for bind in ('move_up','move_down', 'move_left', 'move_right',
                                              'end_text_animation', 'no_task_advance',
                                              'increase_volume', 'decrease_volume')]):
            self.config_dict['controls'] = dict(zip(
                controls.keys(),
